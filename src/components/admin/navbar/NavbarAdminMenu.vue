    ...
    <div v-for="section in store.menuSections" :key="section.title" class="flex flex-col gap-3">
      ...
      <ul class="flex flex-col gap-3">
        <li v-for="item in section.items" :key="item.to">
          <a href="#" @click.prevent="handleClick(item.to)" :class="[store.isCurrent(item.to) ? 'text-black-600 font-semibold' : 'text-gray-700 hover:text-black', 'flex items-center gap-2']">
