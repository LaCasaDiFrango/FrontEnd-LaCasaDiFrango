...
export const useActionSelectorStore = defineStore('actionSelector', () => {
  ...
  const sessionMap = {
    posts: {
      label: 'Lâmina',
      options: {
        'Lâmina': '/dashboard/', // < modificar futuramente
        'Ponto da Lâmina': '/dashboard' // < modificar futuramente      
      }
    },
    quiz: {
      label: 'Pergunta',
      options: {
        'Pergunta': '/dashboard', // < modificar futuramente
        'Resposta': '/dashboard' // < modificar futuramente
      }
    },
    organs: {
      label: 'Órgão',
      options: {
        'Órgão': '/dashboard/' // < modificar futuramente      
        }
    },
    supporting: {
        label: 'Materiais de Apoio',
        options: {
            'Materiais de Apoio': '/dashboard/', // < modificar futuramente
        }
    },
    species: {
      label: 'Espécies',
      options: {
        'Espécies': '/dashboard', // < modificar futuramente
      }
    },
    systems: {
      label: 'Sistema',
      options: {
        'Sistema': '/dashboard', // < modificar futuramente
      }
    },
    users: {
      label: 'Usuário',
      options: {
        'Dados Pessoais': '/dashboard',
      }
    }
  }
  ...
  const sessionKey = computed(() => {
      const match = route.path.match(/^\/dashboard\/([^/]+)/)
